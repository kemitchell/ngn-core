...
var Log = NGN.Class.extend({
	...
	constructor: function(){
		...
		var methods = ['log','info','error','warn','time','timeEnd','trace','assert','dir','debug','flow'];
				...
				value: function(level){
					...
					if (!this.level_cache.hasOwnProperty(level)){
						if (this.level === 'all') {
							...
							// Handle Array
							this.level_cache[level] = this.level.filter(function(el){return el.trim().toLowerCase() === level.trim().toLowerCase();}).length >= 1;
		...
		process.stdout.write(method+' called: ');
					...
					if (console.current_level !== null){
						...
						var curr_level = console.current_level;
